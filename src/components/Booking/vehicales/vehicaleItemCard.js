...
const VehicaleItemCard = ({ productitemid, productitemImagePath , productitem_DESC , productitem_NAME, imgSrc, price , vehicalePrice}) => {
  ...
  let onClickHandler = () => {
    ...
    console.log("productdesc",productitemImagePath)
  }

    return(
        <>
            <div key={productitemid} style={{ display: "flex",  flexWrap:'wrap' , margin:'1.5rem 1.5rem 2.5rem 1.5rem'}} className="onHoverVehicaleEffectDiv">
                  <Link to={`/availablevehicales/${productitemid}`} onClick={onClickHandler} style={{textDecoration:'none' , color:'black'}}>
                    ...
                    <div style={{width:'18rem'}} className="vehicleNameDiv" ><h5 style={{padding:'.5rem' , color:'white' , letterSpacing:'.1rem' , textTransform: 'uppercase' , margin:'0px' , display: "flex", justifyContent:'center' }}>{productitem_NAME.length > 15 ? productitem_NAME.slice(0,15).concat('...') : productitem_NAME}</h5></div>
                      ...
                      className="onHoverVehicaleEffectImage"
                        src={`http://localhost:3335/${productitemImagePath}`}
                      />
                      <div style={{padding:'.4rem .1rem' , color:'white' , textAlign:'center'}}>
                        ...
                        <h6 style={{fontSize:'1.2rem' , margin:'0 .3rem' , letterSpacing:'rem' , color:'#fd7014'}}>{Math.floor(Math.random() * 7000) + 2000} Rs</h6>
                      </div>
                    </div>
                    </Link>
                </div>
